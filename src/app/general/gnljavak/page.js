...
export default function GeneralAAvak() {
  ...
  const [javaktypes, setJavakType] = useState([]);
  ...
  async function fetchJavakTypes() {
    try {
      const response = await fetch("/api/listgeneraljavaktype");
      if (!response.ok) {
        throw new Error("Failed to fetch javaktypes");
      }
      const data = await response.json();
      setJavakType(data);
    } catch (error) {
      console.error("Error fetching javaktypes:", error);
    }
  }
  useEffect(() => {
    fetchJavakTypes(); // Fetch javaktypes when component mounts
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/addgeneraljavak", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          ...
          notes: note,
          type: "debit",
          javaktypeid: selectedVendorId,
          javaktypename: selectedVendor,
        }),
      });
  ...
  return (
    <main className="flex flex-col items-center justify-between p-24">
      ...
      <div className="flex flex-col lg:max-w-5xl lg:w-full lg:mb-3 lg:flex-row lg:justify-between">
        ...
        >
          <option value=""> જનરલ જાવક ક્યાથી ગઈ છે? </option>
          {javaktypes.map((aavaktype) => (
        ...
        <input
          ...
          className="form-control text-black w-full mb-2 md:mb-0"
          placeholder="જનરલ જાવક  અહીંયા લખો."
        />
        <input
          type="text"
          value={note}
      ...
      <h1 className="mb-3 text-2xl font-semibold text-white">જનરલ જાવક યાદી</h1>
